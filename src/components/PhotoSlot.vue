...
<template>
  <figure class="photo-slot" :class="{ 'is-previewable': previewable && loaded && !failed }" :style="{ '--photo-ratio': ratio }" :data-loaded="loaded && !failed">
    ...
    <button v-if="previewable" class="photo-preview-trigger" type="button" :disabled="!loaded || failed" :aria-label="`预览：${alt}`" @click="$emit('preview')"><span aria-hidden="true">⌕</span></button>
  </figure>
</template>